...
export interface JobInsert {
  id: string; source_id: string; next_attempt_at: string; created_at: string
}

export class ProcessingJobsRepo {
  constructor(private db: DB) {}
  insert(row: JobInsert): void {
    this.db.prepare(`INSERT INTO processing_jobs (id,source_id,status,attempts,next_attempt_at,created_at)
      VALUES (@id,@source_id,'pending',0,@next_attempt_at,@created_at)`).run(row)
  }
